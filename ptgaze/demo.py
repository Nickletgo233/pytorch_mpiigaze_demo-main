...
class Demo:
    ...
    def _process_image(self, image) -> None:
        undistorted = cv2.undistort(  # 去畸变处理
            image, self.gaze_estimator.camera.camera_matrix,
            self.gaze_estimator.camera.dist_coefficients)
        self.visualizer.set_image(image.copy())
        faces = self.gaze_estimator.detect_faces(undistorted)  # 检测图像中的面部
        for face in faces:
            R1, T1, face_center = self.gaze_estimator.estimate_gaze(undistorted, face)  # 估计注视向量
            self._draw_face_bbox(face)
            vector2 = self._draw_head_pose(face)
            self._draw_landmarks(face)
            self._draw_face_template_model(face)
            vector = self._draw_gaze_vector(face)
        ...
        try:
            return faces, vector, vector2, R1, T1, face_center
        except:
            pass
    ...
    def _draw_head_pose(self, face: Face) -> None:
        if not self.show_head_pose:
            return
        # Draw the axes of the model coordinate system 绘制模型坐标系的坐标轴
    ...
    def _display_normalized_image(self, face: Face) -> None:
        ...
        elif self.config.mode in ['MPIIFaceGaze', 'ETH-XGaze', 'L2CS']:
    ...
    def _draw_gaze_vector(self, face: Face) -> None:
        ...
        if self.config.mode == 'MPIIGaze':
            for key in [FacePartsName.REYE, FacePartsName.LEYE]:
                ...
                # logger.info(
                #     f'[{key.name.lower()}] pitch: {pitch:.2f}, yaw: {yaw:.2f}')  # 打印注视向量
        elif self.config.mode in ['MPIIFaceGaze', 'ETH-XGaze', 'L2CS']:
            self.visualizer.draw_3d_line(
                face.center, face.center + length * face.gaze_vector)
            pitch, yaw = np.rad2deg(face.vector_to_angle(face.gaze_vector))
            gaze_vector = [pitch, yaw]
            # logger.info(f'[face] pitch: {pitch:.2f}, yaw: {yaw:.2f}')  # 打印注视向量
        else:
            raise ValueError
    ...
    def _draw_fps(self):
        # 计算 FPS（使用 OpenCV 高精度计时）
        if not hasattr(self, '_fps_start_time'):
            self._fps_start_time = cv2.getTickCount()
            self._fps = 0.0
            self._fps_history = []

        end_time = cv2.getTickCount()
        elapsed_time = (end_time - self._fps_start_time) / cv2.getTickFrequency()
        ...
        try:
            ...
            cv2.putText(
                self.visualizer.image,  # 必须是 numpy.ndarray 且 dtype=uint8
            ...
            self.visualizer.image = cv2.cvtColor(self.visualizer.image, cv2.COLOR_RGB2BGR)
        except Exception as e:
            logger.error(f"Failed to draw FPS: {e}")

        # 重置计时器
        self._fps_start_time = end_time