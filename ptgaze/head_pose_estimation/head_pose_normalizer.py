...
class HeadPoseNormalizer:
    """
        几何变换核心：通过旋转 + 缩放 + 透视变换，将任意姿态的人脸/眼睛图像投影到虚拟相机的标准化视角。
        姿态解耦：归一化后的图像和角度消除了原始头部姿态的影响，便于后续算法（如视线估计）处理。
        性能依赖：依赖准确的头部姿态估计（head_pose_rot）和关键点检测（center）。
        参考论文《It’s Written All Over Your Face: Full-Face Appearance-Based Gaze Estimation》（CVPR 2017）
    """
